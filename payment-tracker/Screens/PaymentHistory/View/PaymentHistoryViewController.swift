...
class PaymentHistoryViewController: UIViewController {
    ...
    private func addPayment() {
        let alert = UIAlertController(title: "Novo pagamento", message: "Adicione um novo pagamento", preferredStyle: .alert)
        ...
        alert.addAction(UIAlertAction(title: "Add", style: .default, handler: { [weak self] _ in
            ...
            let card = alert.textFields?[1].text?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            let raw = alert.textFields?[2].text?.replacingOccurrences(of: ",", with: ".").trimmingCharacters(in: .whitespacesAndNewlines)
            guard !name.isEmpty, !card.isEmpty, let raw, let value = Double(raw) else { return }
            
            let payment = Payment(name: name, cardName: card, value: value, date: Date())
    ...
    private func updateTotalInAppGroup() {
        let total = items.reduce(0.0) { $0 + $1.payment.value }
        
        if let groupDefaults = UserDefaults(suiteName: "group.dev.venetikides.paymenttracker") {
